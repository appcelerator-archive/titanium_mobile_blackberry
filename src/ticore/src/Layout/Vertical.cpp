...
ComputedSize Vertical::doVerticalLayout(std::vector<struct Element*> children, double width, double height, bool isWidthSize, bool isHeightSize) {
	...
	for(i = 0; i < len; i++) {
		...
		if (isWidthSize && leftLayoutCoefficients.x1 != 0) {
			...
			measuredSandboxWidth = (*child)._measuredSandboxWidth = sandboxWidthLayoutCoefficients.x1 * width + sandboxWidthLayoutCoefficients.x2 + measuredWidth + (Common::isNaN(measuredLeft) ? 0 : measuredLeft);
	...
	for(i = 0; i < len; i++) {
		...
		measuredLeft = (*child)._measuredLeft = leftLayoutCoefficients.x1 * computedSize.width + leftLayoutCoefficients.x2 * measuredWidth + leftLayoutCoefficients.x3;
									(*child)._measuredSandboxWidth += (Common::isNaN(measuredLeft) ? 0 : measuredLeft);
	}
...
void Vertical::measureNodeForVerticalLayout(struct LayoutProperties layoutProperties, struct Element* element) {
