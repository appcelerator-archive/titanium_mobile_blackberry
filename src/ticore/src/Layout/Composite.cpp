...
struct ComputedSize Composite::doCompositeLayout(std::vector<struct Element*> children, double width, double height, bool isWidthSize, bool isHeightSize) {
	...
	for(i = 0; i < len; i++) {
		...
		if (!(Common::isNaN(minWidthLayoutCoefficients.x1))) {
		...
		(*child)._measuredSandboxWidth = measuredSandboxWidth = sandboxWidthLayoutCoefficients.x1 * height + sandboxWidthLayoutCoefficients.x2 + measuredWidth + (Common::isNaN(measuredLeft) ? 0 : measuredLeft);
		(*child)._measuredSandboxHeight = measuredSandboxHeight = sandboxHeightLayoutCoefficients.x1 * height + sandboxHeightLayoutCoefficients.x2 + measuredHeight + (Common::isNaN(measuredTop) ? 0 : measuredTop);
...
void Composite::measureNodeForCompositeLayout(struct LayoutProperties layoutProperties, struct Element* element) {
