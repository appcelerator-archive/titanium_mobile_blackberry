...
{
	HandleScope scope;
	getJSObjectTemplate()->Set(String::New(name), obj, DontDelete);
}

void Ti::TiModule::addString(QString name, QString value)
{
	HandleScope scope;
	getJSObjectTemplate()->Set(String::New(name.toLocal8Bit().data()), String::New(value.toLocal8Bit().data()), DontDelete);
}

void Ti::TiModule::addNumber(QString name, double value)
{
	HandleScope scope;
	getJSObjectTemplate()->Set(String::New(name.toLocal8Bit().data()), Number::New(value), DontDelete);
}
